...
class FPGrowth():
    ...
    def __init__(self, dataset, min_sup=0.0):
        ...
        self.freq_L1 = {}  # 1-频繁项
        self.freq_item_sets = []  # 存储每个频繁项及其对应的计数

    @classmethod
    def __get_frequency(cls, trans_records):
        rect = {}
    ...
    def __fpgrowth(self, cpb, post_model):
        ...
        for header in headers.values():
            ...
            self.freq_item_sets.append(temp)
            # 新的后缀模式：表头项+上一次的后缀模式（注意保持顺序，始终按F1的顺序排列）
            new_post_pattern = [header.name]
    ...
    def __add_nodes(self, ancestor, record, headers):
        while len(record) > 0:
            item = record.pop(0)
            if item in headers:
                leaf_node = FPTreeNode(item)
                leaf_node.count = 1
                leaf_node.parent = ancestor
                ancestor.add_child(leaf_node)
                header = headers[item]
                tail = header.tail
                if tail is None:
                    header.next_homonym = leaf_node
                else:
                    tail.next_homonym = leaf_node
                header.tail = leaf_node
                self.__add_nodes(leaf_node, record, headers)
